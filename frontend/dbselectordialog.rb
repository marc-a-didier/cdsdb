...
class DBSelectorDialog
    ...
    def do_op_delete()
        ...
        return if iter.nil?
        count = DBIntf::connection.get_first_value("SELECT COUNT(rrecord) FROM records WHERE #{@tbl_ref}=#{iter[0]}")
        if count > 0
    ...
    def run(tbl_id)
        @tbl_name = tbl_id+"s"
        @tbl_ref  = "r"+tbl_id
        #DBIntf::connection.execute( "SELECT * FROM #{@tbl_name} WHERE #{@tbl_ref} > 1;" ) do |row|
        DBIntf::connection.execute( "SELECT * FROM #{@tbl_name};" ) do |row|
            iter = @tv.model.append()
            iter[0] = row[0]
            iter[1] = row[1]
        end
