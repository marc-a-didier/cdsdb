...
# rescue LoadError
	puts "Translations disabled by mad"
	def _(txt)
		txt
	end
# end
...
puts line # !!mad
			if line[0,5] =~ /\s+\d+\./
                ...
                @mSecLength << lengthText[1,2].to_i*60*1000+lengthText[4,5].to_i*1000+lengthText[7,8].to_i*10 # !!mad
			elsif line =~ /CDROM\D*:/
				@devicename = $'.strip()
			elsif line[0,5] == "TOTAL"
				@playtime = line.split()[2][1,5]
                len = line.split()[2] # !!mad
print "len=",len,"\n" # !!mad
