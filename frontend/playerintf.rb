...
module BrowserPlayerIntf
            ...
            if iter[link_index].playable? # OK or MISPLACED
                ...
                if @mc.tasks.track_in_download?(iter[link_index])
                    ...
                    if downloads_count < max_downloads && iter[link_index].available_on_server?
                        iter[link_index].get_remote_audio_file(self, @mc.tasks)
                        downloads_count += 1
                    end
                end
            end
    ...
    # Returns the PlayerData corresponnding to the message.
    ...
    def get_audio_file(tv, link_index)
        while true
            ...
            tv.set_cursor(iter.path, nil, false)
            if iter[link_index].get_audio_file(self, @mc.tasks) == Audio::Status::NOT_FOUND
                @track_ref += 1
            else
                break
            end
        ...
        return PlayerData.new(self, @track_ref, iter[link_index])
    end
