...
class DBReorderer
    ...
    def initialize
        ...
        @outfile = File.new(Cfg::instance.database_dir+"reorder.sql", "w")
    end

    def dup_table(table) # Copy table as it, that is there are no change
        DBIntf::connection.execute("SELECT * FROM #{table}") do |row|
...
#             end
    ...
    def process_covers
        Dir[Cfg::instance.covers_dir+"*"].each { |entry|
            ...
            if File::directory?(entry)
                ...
                if dname.to_i > 0
                    cmd = "mkdir #{Cfg::instance.rsrc_dir}newcovers/#{@rec_map[dname.to_i]}"
                    #@outfile.puts(cmd)
                    system(cmd)
                end
                Dir[entry+"/*"].each { |subent|
                    ...
                    if name.to_i > 0
                        cmd = "cp #{subent} #{Cfg::instance.rsrc_dir}newcovers/#{@rec_map[dname.to_i]}/#{@trk_map[name.to_i]}#{File::extname(subent)}"
                        #@outfile.puts(cmd)
                        system(cmd)
                    end
                }
                ...
                if name.to_i > 0
                    cmd = "cp #{entry} #{Cfg::instance.rsrc_dir}newcovers/#{@rec_map[name.to_i]}#{File::extname(entry)}"
                    #@outfile.puts(cmd)
                    system(cmd)
                end
            end
    ...
    def process_plists
        ...
        DBIntf::connection.execute("SELECT * FROM plists ORDER BY LOWER(sname);") { |row|
            i += 1
            ...
            DBIntf::connection.execute("SELECT * FROM pltracks WHERE rplist=#{old_pl} ORDER BY iorder;") { |row2|
                j += 1
    ...
    def process_records
        DBIntf::connection.execute("SELECT * FROM records WHERE rartist=#{@old_art} ORDER BY LOWER(stitle);") { |row|
            ...
            @rec_nsegs = DBIntf::connection.get_first_value("SELECT COUNT(rsegment) FROM segments WHERE rrecord=#{@old_rec};")
    ...
    def run
        ...
        DBIntf::connection.execute("SELECT * FROM artists WHERE rartist > 0 ORDER BY LOWER(sname);") { |row|
