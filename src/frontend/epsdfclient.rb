...
module Epsdf
    ...
    class Client
        ...
        def task_to_params(task)
            params = { 'type' => task.resource_type, 'block_size' => Cfg.tx_block_size }

            if task.resource_type == :track
                params['file_name'] = Audio::Link.new.set_track_ref(task.resource_data.track.rtrack).setup_audio_file.file
                params['rtrack'] = task.resource_data.track.rtrack
            elsif task.resource_type == :db
                params['file_name'] = DBIntf.build_db_name
            else
                params['file_name'] = task.resource_data
            end
            params['file_name'] = Cfg.relative_path(task.resource_type, params['file_name'])
        ...
        def upload_resource(network_task)
            status = false
            msg = setup_file_properties_msg(task_to_params(network_task))
        ...
        def process_task(task)
            Cfg.remote? ? (task.action == :upload ? upload_resource(task) : download_resource(task)) : false
        end
