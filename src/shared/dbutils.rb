...
module DBUtils
    ...
    def self.client_sql(sql)
        self.log_exec(sql)
        Epsdf::Client.new.exec_sql(sql) if Cfg.remote?
    end

    def self.threaded_client_sql(sql)
        self.log_exec(sql)
        Thread.new { Epsdf::Client.new.exec_sql(sql) } if Cfg.remote?
    end
    ...
    def self.exec_batch(sql, host, log = true)
        ...
        Epsdf::Client.new.exec_batch(sql) if Cfg.remote?
        # May be dangerous to spawn a thread... if request made on the record being inserted,
        # don't know what happen...
#         Thread.new { Epsdf::Client.new.exec_batch(sql) } if Cfg.remote?
    end
